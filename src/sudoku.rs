...
pub type Digit = NonZeroU8; // TODO may use an enum with 9 variants if we're feeling extra spicy
...
impl Grid {
    ...
    // TODO better error
    pub fn from_string(spec: &str) -> Result<Grid, &'static str> {
    ...
    pub fn is_row_ok(&self, r: u8) -> bool {
        cells_ok(&self.cells[r as usize])
    }

    pub fn is_col_ok(&self, c: u8) -> bool {
        cells_ok(self.cells.iter().map(|row| &row[c as usize]))
    }

    pub fn is_house_ok(&self, h: u8) -> bool {
